...
class PlayerStats:
    ...
    def add_player(self, player_name, matches, goals, assists,
        ...
        try:
            ...
            if not player_name.replace(" ", "").isalpha():
                raise ValueError(
                    "Player name must only contain letters and spaces."
                    )
            ...
            for value in inputs:
                if not value.isdigit() or int(value) < 0:
                    raise ValueError(
                        "All statistics must be positive numbers."
                        )
            ...
            new_data = {
                ...
                'Goals': int(goals),
                'Assists': int(assists),
                'Yellow Cards': int(yellow_cards),
                'Red Cards': int(red_cards)
            }
            self.stats = pd.concat(
                [self.stats, pd.DataFrame([new_data])], ignore_index=True
            )
            print(f"{player_name}'s data added successfully!")

            # Append the player stats to Google Sheet
            row = [player_name, matches, goals, assists,
                   yellow_cards, red_cards]
    ...
    def get_player_stats(self, player_name):
        ...
        player_data = stats_df[
            stats_df['Player'].str.lower() == player_name.lower()]
    ...
    def get_top_players(self):
        ...
        def find_top_players(column_name):
            ...
            top_players = stats_df[
                stats_df[column_name] == max_value]['Player'].tolist()
            return ', '.join(top_players) if top_players else "No top player"
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            # Add player data
            tracker.add_player(
                name, matches, goals, assists, yellow_cards, red_cards
            )
